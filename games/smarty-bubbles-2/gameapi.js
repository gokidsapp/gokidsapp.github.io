  ...
  module.prototype.getAdTagUrl = function (variant, priceMode, waterfallID) {
    ...
    if (detection.is.iframe) {
      if (M.mcm_required) {
        ...
        var matchedOrigin = self.config.last_ancestor_matched
          ? self.config.last_ancestor_origin
          : self.config.first_ancestor_matched
          ? self.config.first_ancestor_origin
          : self.config.referrer_matched
          ? self.config.referrer_origin
          : "";
        if (
          typeof matchedOrigin == "string" &&
    ...
    window.googletag.cmd.push(function () {
      M.rewardedAd.slot = window.googletag
        ...
        .setTargeting(
          "adinterval",
          M.lastAdCall > 0
            ? "" + Math.floor((self.now() - M.lastAdCall) / 10000) * 10
            : ""
        );
